...
def find_outliers_quantile(data, feature, left=0.01, right=0.99):
    """
    Находит выбросы в данных, используя данные и признак, по которому будет совершаться поиск выбросов,
    а также есть границы(квантили), квантили какого-то определенного уровня.
    
    В качестве левой границы выбирается квантиль уровня 1, а в качестве правой 
    квантиль уровня 99. По итогу те наблюдения, которые не попали в интервал
    обозначенные данными квантилями считаются выбросами.
    Данная функция будет отсекать 2% наблюдений(слева 1% и справа1%),
    таким образом производить очистку.

    Args:
        data (pandas.DataFrame): набор данных
        feature (str): имя признака, на основе которого происходит поиск выбросов
        left (float, optional): количество стандартных отклонений в левую сторону распределения. 
        right (float, optional): количество стандартных в правую сторону распределения.
